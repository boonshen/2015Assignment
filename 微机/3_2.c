...
int dac_increment = 1;								//increment value for dac_val
char dac_val = 0;									//output value of dac0
...
void main(){
	...
	ADCCON2 = 0x01;	//configure adc register to select channel 1
	
	DACCON = 0x8D;	//10001101, select dac0, asynchronos 8bit output from 0 to 2.5V 
	...
	while(1) {				//dac output before adc read from dac
		
		DAC0L = dac_val;	//dac output
		dac_val = dac_val + dac_increment;
		if(dac_val == 255 || dac_val == 0) dac_increment = -1 * dac_increment;
		delayMillisecond(1);
